...
import { Openregister, ClientOptions } from 'openregister';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Openregister)`
    : `const run: (${functionSource.client}: Openregister) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
