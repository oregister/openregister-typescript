...
export const handler = async (client: Openregister, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.person.getDetailsV1(person_id)));
  } catch (error) {
