...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['query: string;'],
    response:
      '{ results: { active: boolean; address: object; company_id: string; country: string; legal_form: company_legal_form; name: string; purpose: string; register_court: string; register_number: string; register_type: company_register_type; }[]; }',
    markdown:
  ...
  {
    name: 'find_insolvencies_v1',
    endpoint: '/v1/search/insolvency',
    httpMethod: 'post',
    summary: 'Search for insolvency proceedings',
    description: 'Search for insolvency proceedings',
    stainlessPath: '(resource) search > (method) find_insolvencies_v1',
    qualified: 'client.search.findInsolvenciesV1',
    params: [
      'filters?: { keywords?: string[]; max?: string; min?: string; value?: string; values?: string[]; }[];',
      'pagination?: { page?: number; per_page?: number; };',
      'query?: { value: string; };',
    ...
    response:
      "{ pagination: { page: number; per_page: number; total_pages: number; total_results: number; }; results: { id: string; administration_kind: 'external_administration' | 'self_administration' | 'protective_shield'; administrator_name: string; case_number: string; city: string; closed_at: string; company_id: string; court: string; current_status: string; debtor_kind: 'legal_person' | 'natural_person'; debtor_legal_form: string; debtor_name: string; has_open_insolvency: boolean; insolvency_grounds: string[]; last_event_at: string; opened_at: string; person_id: string; proceeding_kind: 'regular_insolvency' | 'consumer_insolvency'; }[]; }",
    markdown:
      "## find_insolvencies_v1\n\n`client.search.findInsolvenciesV1(filters?: { keywords?: string[]; max?: string; min?: string; value?: string; values?: string[]; }[], pagination?: { page?: number; per_page?: number; }, query?: { value: string; }): { pagination: pagination; results: object[]; }`\n\n**post** `/v1/search/insolvency`\n\nSearch for insolvency proceedings\n\n### Parameters\n\n- `filters?: { keywords?: string[]; max?: string; min?: string; value?: string; values?: string[]; }[]`\n  Filters to filter insolvency proceedings.\n\n\n- `pagination?: { page?: number; per_page?: number; }`\n  Pagination parameters.\n\n  - `page?: number`\n    Page number to return.\n\n  - `per_page?: number`\n    Number of results per page.\n\n\n- `query?: { value: string; }`\n  Search query to filter insolvency proceedings.\n\n  - `value: string`\n    Search query to filter insolvency proceedings.\nMatches against debtor name, case number, administrator name and court.\n\n\n### Returns\n\n- `{ pagination: { page: number; per_page: number; total_pages: number; total_results: number; }; results: { id: string; administration_kind: 'external_administration' | 'self_administration' | 'protective_shield'; administrator_name: string; case_number: string; city: string; closed_at: string; company_id: string; court: string; current_status: string; debtor_kind: 'legal_person' | 'natural_person'; debtor_legal_form: string; debtor_name: string; has_open_insolvency: boolean; insolvency_grounds: string[]; last_event_at: string; opened_at: string; person_id: string; proceeding_kind: 'regular_insolvency' | 'consumer_insolvency'; }[]; }`\n\n  - `pagination: { page: number; per_page: number; total_pages: number; total_results: number; }`\n  - `results: { id: string; administration_kind: 'external_administration' | 'self_administration' | 'protective_shield'; administrator_name: string; case_number: string; city: string; closed_at: string; company_id: string; court: string; current_status: string; debtor_kind: 'legal_person' | 'natural_person'; debtor_legal_form: string; debtor_name: string; has_open_insolvency: boolean; insolvency_grounds: string[]; last_event_at: string; opened_at: string; person_id: string; proceeding_kind: 'regular_insolvency' | 'consumer_insolvency'; }[]`\n\n### Example\n\n```typescript\nimport Openregister from 'openregister';\n\nconst client = new Openregister();\n\nconst response = await client.search.findInsolvenciesV1();\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.search.findInsolvenciesV1',
        example:
          "import Openregister from 'openregister';\n\nconst client = new Openregister({\n  apiKey: process.env['OPENREGISTER_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.search.findInsolvenciesV1();\n\nconsole.log(response.pagination);",
      },
      python: {
        method: 'search.find_insolvencies_v1',
        example:
          'import os\nfrom openregister import Openregister\n\nclient = Openregister(\n    api_key=os.environ.get("OPENREGISTER_API_KEY"),  # This is the default and can be omitted\n)\nresponse = client.search.find_insolvencies_v1()\nprint(response.pagination)',
      },
      java: {
        method: 'search().findInsolvenciesV1',
        example:
          'package com.openregister.api.example;\n\nimport com.openregister.api.client.OpenregisterClient;\nimport com.openregister.api.client.okhttp.OpenregisterOkHttpClient;\nimport com.openregister.api.models.search.SearchFindInsolvenciesV1Params;\nimport com.openregister.api.models.search.SearchFindInsolvenciesV1Response;\n\npublic final class Main {\n    private Main() {}\n\n    public static void main(String[] args) {\n        OpenregisterClient client = OpenregisterOkHttpClient.fromEnv();\n\n        SearchFindInsolvenciesV1Response response = client.search().findInsolvenciesV1();\n    }\n}',
      },
      go: {
        method: 'client.Search.FindInsolvenciesV1',
        example:
      ...
      cli: {
        method: 'search find_insolvencies_v1',
        example: "openregister search find-insolvencies-v1 \\\n  --api-key 'My API Key'",
      },
      http: {
        example:
          "curl https://api.openregister.de/v1/search/insolvency \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $OPENREGISTER_API_KEY\" \\\n    -d '{}'",
      },
    },
  },
  {
  ...
  },
  {
    ...
    params: ['company_id: string;'],
    response: '{ indicators: object[]; merged: object; reports: object[]; }',
    markdown:
      "## get_financials_v1\n\n`client.company.getFinancialsV1(company_id: string): { indicators: object[]; merged: object; reports: object[]; }`\n\n**get** `/v1/company/{company_id}/financials`\n\nGet financial reports\n\n### Parameters\n\n- `company_id: string`\n\n### Returns\n\n- `{ indicators: { active_accruals: number; affiliated_liabilities: number; balance_sheet_total: number; bank_debt: number; capital_reserves: number; cash: number; commission_expense: number; commission_income: number; current_assets: number; date: string; ebit: number; ebitda: number; employees: number; equity: number; financial_assets: number; financial_debt: number; financial_depreciation: number; fixed_assets: number; gross_profit: number; income_after_tax: number; income_before_tax: number; intangible_assets: number; interest_expense: number; interest_income: number; inventory: number; liabilities: number; materials: number; net_income: number; operating_depreciation: number; other_liabilities: number; other_operating_expenses: number; other_operating_income: number; other_provisions: number; other_taxes: number; parent_net_income: number; passive_accruals: number; pension_provisions: number; profit_carryforward: number; provisions: number; real_estate: number; receivables: number; report_id: string; retained_earnings: number; revenue: number; salaries: number; shareholder_liabilities: number; tangible_assets: number; taxes: number; trade_payables: number; trade_receivables: number; }[]; merged: { aktiva: object; passiva: object; guv?: object; }; reports: { aktiva: object; consolidated: boolean; passiva: object; report_end_date: string; report_id: string; report_start_date: string; sources: { html_url: string; }[]; guv?: object; }[]; }`\n\n  - `indicators: { active_accruals: number; affiliated_liabilities: number; balance_sheet_total: number; bank_debt: number; capital_reserves: number; cash: number; commission_expense: number; commission_income: number; current_assets: number; date: string; ebit: number; ebitda: number; employees: number; equity: number; financial_assets: number; financial_debt: number; financial_depreciation: number; fixed_assets: number; gross_profit: number; income_after_tax: number; income_before_tax: number; intangible_assets: number; interest_expense: number; interest_income: number; inventory: number; liabilities: number; materials: number; net_income: number; operating_depreciation: number; other_liabilities: number; other_operating_expenses: number; other_operating_income: number; other_provisions: number; other_taxes: number; parent_net_income: number; passive_accruals: number; pension_provisions: number; profit_carryforward: number; provisions: number; real_estate: number; receivables: number; report_id: string; retained_earnings: number; revenue: number; salaries: number; shareholder_liabilities: number; tangible_assets: number; taxes: number; trade_payables: number; trade_receivables: number; }[]`\n  - `merged: { aktiva: { rows: object[]; }; passiva: { rows: object[]; }; guv?: { rows: object[]; }; }`\n  - `reports: { aktiva: { rows: object[]; }; consolidated: boolean; passiva: { rows: object[]; }; report_end_date: string; report_id: string; report_start_date: string; sources: { html_url: string; }[]; guv?: { rows: object[]; }; }[]`\n\n### Example\n\n```typescript\nimport Openregister from 'openregister';\n\nconst client = new Openregister();\n\nconst response = await client.company.getFinancialsV1('company_id');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.openregister.de/v1/credits \\\n    -H "Authorization: Bearer $OPENREGISTER_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.openregister.de/v1/insolvency/$INSOLVENCY_ID \\\n    -H "Authorization: Bearer $OPENREGISTER_API_KEY"',
      },
    },
  },
];
