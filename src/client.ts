...
import {
  ...
  SearchLookupCompanyByNameParams,
  SearchLookupCompanyByNameResponse,
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['OPENREGISTER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['OPENREGISTER_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Openregister API.
 */
export class Openregister {
   ...
   */
  constructor({
    baseURL = readEnv('OPENREGISTER_BASE_URL'),
    apiKey = readEnv('OPENREGISTER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.OpenregisterError(
        "The OPENREGISTER_API_KEY environment variable is missing or empty; either provide it, or instantiate the Openregister client with an apiKey option, like new Openregister({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.openregister.de`,
    };
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('OPENREGISTER_LOG'), "process.env['OPENREGISTER_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.openregister.de';
  }
